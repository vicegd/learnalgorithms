...
Constraint satisfaction problems are a natural fit for backtracking because the goal is to assign values to variables while satisfying a set of explicit constraints, and each partial assignment can usually be checked immediately for consistency. In other backtracking problems, the search may be driven by a different objective, such as finding a path, generating all combinations, or optimizing a score, so the feasibility test and pruning rules are often less uniform and may depend more on the specific structure of the problem.

Typical examples of CSPs include:
- N-Queens,
- graph coloring,
- Sudoku,
...
This scheme is extremely general and appears in most classical examples, such as N-Queens, Sudoku, subset generation, graph coloring, and Hamiltonian path search. However, although the backtracking skeleton is always similar, each problem defines its own:
